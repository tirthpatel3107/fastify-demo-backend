...
export default async function webhookRoutes(fastify: FastifyInstance) {
  ...
  fastify.patch('/logs/:id/process', catchAsync(WebhookController.markWebhookLogAsProcessed));
  ...
  fastify.get('/logs/unprocessed', catchAsync(WebhookController.getUnprocessedWebhookLogs));

  // Get webhook logs by prescription ID
  fastify.get('/logs/prescription/:prescriptionId', catchAsync(WebhookController.getWebhookLogsByPrescription));

  // Get webhook logs by event type
  fastify.get('/logs/event/:eventType', catchAsync(WebhookController.getWebhookLogsByEventType));

  // Process webhook event
  fastify.post('/process', catchAsync(WebhookController.processWebhookEvent));

  // Clean up old webhook logs
  fastify.delete('/logs/cleanup', catchAsync(WebhookController.cleanupOldWebhookLogs));
