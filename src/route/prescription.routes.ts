...
export default async function prescriptionRoutes(fastify: FastifyInstance) {
  ...
  fastify.patch(
    "/:id/status",
    catchAsync(PrescriptionController.updatePrescriptionStatus),
  );
  ...
  fastify.get(
    "/doctor/:doctorId",
    catchAsync(PrescriptionController.getPrescriptionsByDoctor),
  );

  // Get prescriptions by status
  fastify.get(
    "/status/:status",
    catchAsync(PrescriptionController.getPrescriptionsByStatus),
  );

  // Search prescriptions by patient name
  fastify.get(
    "/search/patient/:patientName",
    catchAsync(PrescriptionController.searchPrescriptionsByPatient),
  );

  // Get prescription statistics
  fastify.get(
    "/stats/overview",
    catchAsync(PrescriptionController.getPrescriptionStats),
  );
}
