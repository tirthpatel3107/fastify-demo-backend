...
export default async function tokenRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/access/:accessToken', catchAsync(TokenController.getTokenByAccessToken));
  ...
  fastify.delete('/user/:userId', catchAsync(TokenController.deleteTokensByUserId));

  // Refresh token
  fastify.post('/refresh', catchAsync(TokenController.refreshToken));

  // Validate token
  fastify.get('/validate/:accessToken', catchAsync(TokenController.validateToken));

  // Clean up expired tokens
  fastify.delete('/cleanup/expired', catchAsync(TokenController.cleanupExpiredTokens));
}
